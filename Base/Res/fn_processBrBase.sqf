...
while { alive _building } do {
	...
	if (!_patrolTeamAlive) then {
        ...
        [_side, _templateToUse, _building, Format ["%1%2TeamBaseInfantryPatrol%3", _sideText, _facIndex, 1], true] spawn WFSE_FNC_processResTeam;
	...
	if (!_patrolTeamAlive) then {
        _ranTemp = _lightTemplate # (random((count _lightTemplate)-1));
        _templateToUse = _resTemplates # _ranTemp;
        [_side, _templateToUse, _building, Format ["%1%2TeamBaseInfantryPatrol%3", _sideText, _facIndex, 2], true] spawn WFSE_FNC_processResTeam;
