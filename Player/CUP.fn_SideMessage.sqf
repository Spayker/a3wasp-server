...
switch (true) do {
	...
	case (_message in ["Constructed","Destroyed","Deployed","Mobilized","IsUnderAttack"]): {
		...
		if ((_parameters # 0 ) == "Base") then {
			...
			_speaker kbTell [_receiver, _topicSide, "simpletwo",["1","",_localizedString,[format ["\CUP\Dubbing%1warfare\%3.%2", _announcerType # 0, _announcerType # 1, _value]]],
			["2","",localize (format["STR%1", _message]),[format ["\CUP\Dubbing%1warfare\%3.%2", _announcerType # 0, _announcerType # 1, _message]]], true];
		} else {
			...
			["1","",localize "STR_Town",[format ["\CUP\Dubbing%1core\maploc\%3.%2", _announcerType # 0, _announcerType # 1, "Town"]]],
			["2","",_localizedString,[]],
			["3","",localize (format["STR%1", _message]),[format ["\CUP\Dubbing%1warfare\%3.%2", _announcerType # 0, _announcerType # 1, _message]]], true];
		};
	};
